...
- **Backend**: Flask
- **Database**: SQLite (accessed via MCP module only)
- **LLM**: Gemini 2.5 (vision capabilities for PDF processing)
- **Language**: Python
...
1. **MCP Module** (`mcp/`)
   - Sole interface to SQLite database
   - Flask app must NOT access database directly
   - All CRUD operations go through MCP
...
3. **Processing Module** (`processing/`)
   - LLM vision extraction from PDFs
   - Test data parsing: date, lab name, values, limits, documentation
   - Test normalization (see below)
...
**Documents**
- file path, content hash, user_id, upload date
...
**Test Results**
- test_id, user_id, document_id
- lab_test_name (original name from lab)
- value, unit, lower_limit, upper_limit
- test_date, lab_name
...
> - the app will have an mcp module for working with the database (aka the app will not access directly the database)
